...
vis_tmp_path = "/home/cedric/PERCEPTION_CAMERA/Deep_Learning/RektNet/tmp/detect/" #!!!don't specify this path outside of /tmp/, otherwise important files could be removed!!!
...
def vis_kpt_and_save(np_image, image_name, h_scale, w_scale, labels, color=(52,31,163)):
    ...
    for pt in np.array(labels):
        x_coor, y_coor = pt 
        cv2.circle(np_image, (int(x_coor), int(y_coor)), circ_size, color, -1) #BGR color (52,31,163) is called mit logo red
...
def visualize_data(images, labels):
    ...
    for index in  range(len(images)):
        ...
        image = cv2.imread("/home/cedric/PERCEPTION_CAMERA/Deep_Learning/RektNet/dataset/RektNet_Dataset/"+images[index])
        h, w, _ = image.shape
        dim_diff = np.abs(h - w)
        if h<=w:
        ...
        os.rename(vis_tmp_path + images[index], vis_path + "/vis_data_output/" + images[index])
        cv2.waitKey(0)
        for i in range(hm.shape[0]):
            cv2.imwrite(vis_tmp_path + images[index], image)
            os.rename(vis_tmp_path + images[index], vis_path + "/vis_data_output/" + images[index])
...
def load_train_csv_dataset(train_csv_uri, validation_percent, keypoint_keys, dataset_path, cache_location=None):
    ...
    # # Split for train/val.
    '''
    all_examples = np.arange(0,num_train)

    random.shuffle(all_examples)
    val_idcs = all_examples[0:num_val//2]
    test_idcs = all_examples[num_val//2:num_val]
    train_idcs = all_examples[num_val:]
    '''
    '''
    val_labels = train_labels[0:num_val//2]
    val_images = train_images[0:num_val//2]
    test_images = train_images[num_val//2:num_val]
    test_labels = train_labels[num_val//2:num_val]
    train_labels = train_labels[num_val:]
    train_images = train_images[num_val:]
    '''
    val_labels = train_labels[0:50]
    val_images = train_images[0:50]
    test_images = train_images[50:100]
    test_labels = train_labels[50:100]
    train_labels = train_labels[100:]
    train_images = train_images[100:]
    
    #val_images, val_labels, test_images, test_labels = split_train_test(val_images,val_labels, 0.5)
    print(f"training image number: {len(train_images)}")
    print(f"validation image number: {len(val_images)}")
    print(f"Test image number: {len(test_images)}")
...
def calculate_mean_distance(epoch_kpt_dis):
    ...
    mid_L_top_max = np.amax(mid_L_top)
    mid_L_top = np.mean(mid_L_top)

    mid_R_top_std = np.std(mid_R_top)
    mid_R_top_max = np.amax(mid_R_top)
    mid_R_top = np.mean(mid_R_top)
        
    mid_L_bot_std = np.std(mid_L_bot)
    mid_L_bot_max = np.amax(mid_L_bot)
    mid_L_bot = np.mean(mid_L_bot)

    mid_R_bot_std = np.std(mid_R_bot)
    mid_R_bot_max = np.amax(mid_R_bot)
    mid_R_bot = np.mean(mid_R_bot)
    
    bot_L_std = np.std(bot_L)
    bot_L_max = np.amax(bot_L)
    bot_L = np.mean(bot_L)

    bot_R_std = np.std(bot_R)
    bot_R_max = np.amax(bot_R)
    bot_R = np.mean(bot_R)

    #total adds all means 
    total = top + mid_L_top + mid_R_top + mid_L_bot + mid_R_bot + bot_L + bot_R

    return [top,mid_L_top,mid_R_top,mid_L_bot,mid_R_bot,bot_L,bot_R],total,[top_std,mid_L_top_std,mid_R_top_std,mid_L_bot_std,mid_R_bot_std,bot_L_std,bot_R_std], [top_max,mid_L_top_max,mid_R_top_max,mid_L_bot_max,mid_R_bot_max,bot_L_max,bot_R_max]
