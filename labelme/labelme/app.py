...
img_dir = os.path.expanduser('~/CATKIN_FS/src/02_perception/camera/training/RektNet/dataset/unlabeled/')
model = KeypointNet()
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        imagePredict = action(self.tr('&Image Predict'),
                       self.imagePredict,
                       icon='pred',
                       tip=self.tr('Automatic Labeling of Picture'),enabled=False)

        cudaEnable = action(self.tr('&Cuda Enable'),
                       self.cudaEnable,
                       icon='nvidia',
                       tip=self.tr('Set Cuda as device'),enabled=False)
    ...
    def imagePredict(self):
        ...
        if self.filename :
           ...
           if not img_name in self.json_list:
                ...
                img = prep_image(image=img,target_image_size=img_size)
                img = (img.transpose((2,0,1))/255)[np.newaxis,:]
    ...
    def importDroppedImageFiles(self, imageFiles):
        ...
        if len(self.imageList) > 1:
            ...
            self.actions.imagePredict.setEnabled(True)
            self.actions.cudaEnable.setEnabled(True)
